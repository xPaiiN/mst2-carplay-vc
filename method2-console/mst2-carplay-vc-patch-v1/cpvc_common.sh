#!/bin/sh
# cpvc_common.sh -- shared helpers for the MANUAL (self-contained on-SD) MST2 CarPlay-VC
# deploy scripts. Semantically identical to the GreenMenu custom/greenmenu/scripts/cpvc_common.sh,
# EXCEPT for two SD-layout adaptations:
#   * the "stick" is the directory that holds THESE scripts (+ the ONE mst2-carplay-vc-*.jar and the
#     cpvc-cover-bridge), not a custom/java/ subtree -> find_stick derives it from $0 with
#     parameter expansion (no external dirname/basename -- both are absent on the stock image);
#   * no ESD_SCRIPTS / GreenMenu dependency (fully self-contained on the SD).
# On-car whitelist only (ls cp mv mount awk grep sed rm mkdir echo [ test date sleep slay chmod);
...
# Locate the toolbox stick. MANUAL method: the stick is the directory that holds THIS script set
# (the ONE mst2-carplay-vc-*.jar + cpvc-cover-bridge live right next to the scripts). We derive it from
# $0 using parameter expansion only (whitelist-safe -- no external dirname/basename), resolve it
# to an absolute path when possible (cd+pwd are shell built-ins), and confirm it carries a patch
# jar. If $0 cannot be resolved to a jar-bearing dir, fall back to the default SD mounts
# (project mount-race rule: try mp001 first, then mp000).
find_stick() {
    case "$0" in
        */*) _sd="${0%/*}" ;;
        *)   _sd="." ;;
    ...
    case "$_sd" in
        /*) ;;                                            # already absolute
        *)  _abs=$(cd "$_sd" 2>/dev/null && pwd); [ -n "$_abs" ] && _sd="$_abs" ;;
    esac
    for _j in "$_sd"/mst2-carplay-vc-*.jar; do
        if [ -f "$_j" ]; then
            STICK="$_sd"; SD_BACKUP="$_sd/backup/carplayvc"; return 0
        fi
    done
    for _base in "${CPVC_ROOT}/media/mp001" "${CPVC_ROOT}/media/mp000"; do
        for _dir in "$_base" "$_base"/*; do
            [ -d "$_dir" ] || continue
            for _j in "$_dir"/mst2-carplay-vc-*.jar; do
                if [ -f "$_j" ]; then
                    STICK="$_dir"; SD_BACKUP="$_dir/backup/carplayvc"; return 0
                fi
            done
        done
    done
...
find_patch_jar() {
    ...
    _found=""
    _count=0
    for j in "$STICK"/mst2-carplay-vc-*.jar; do
        [ -f "$j" ] || continue
        _found="$j"
        _count=$((_count + 1))
    done
