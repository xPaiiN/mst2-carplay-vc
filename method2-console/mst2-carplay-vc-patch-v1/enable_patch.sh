...
for a in "$@"; do
    case "$a" in
        --debug) DEBUG=yes ;;
        *) echo "WARN: ignoring unknown argument '$a' (only --debug is supported)" ;;
    esac
done

echo "mst2-carplay-vc enable --debug=$DEBUG"

find_stick || { echo "FAIL: No mst2-carplay-vc-*.jar found in sd/custom/java"; exit 1; }
find_patch_jar || exit 1
remount_rw
echo " [OK]   sd=$STICK"
...
COVER_SRC="$STICK/cpvc-cover-bridge"
[ -s "$PATCH_JAR" ] || { echo "FAIL: jar is invalid/empty: $PATCH_JAR"; exit 1; }
if [ "$USE_COVER" = yes ] && [ ! -s "$COVER_SRC" ]; then
    echo "FAIL: cpvc-cover-bridge missing (required for $VARIANT)."; exit 1
fi
...
if ! { cp "$PATCH_JAR" "$STATE/.jar.tmp" && rm -f "$JAR_DEST" && mv "$STATE/.jar.tmp" "$JAR_DEST"; }; then
    echo "FAIL: jar copy failed."; exit 1
fi
...
if [ "$USE_COVER" = yes ]; then
    ...
    if ! { cp "$COVER_SRC" "$STATE/.cb.tmp" && rm -f "$COVER_DEST" && mv "$STATE/.cb.tmp" "$COVER_DEST"; }; then
        echo "FAIL: cpvc-cover-bridge copy failed."; exit 1
    fi
    chmod 755 "$COVER_DEST" 2>/dev/null
    [ -x "$COVER_DEST" ] || { echo "FAIL: cover-bridge not executable after chmod. Aborting."; exit 1; }
...
if [ "${LAST:-0}" -le 0 ]; then
    echo "FAIL: no BOOTCLASSPATH line in runHMI.sh. Aborting."; rm -f "$STATE/.rh.stripped"; exit 1
fi
if [ "$USE_COVER" = yes ]; then
...
if ! sh -n "$STATE/.rh.patched" 2>/dev/null; then
    echo "FAIL: patched runHMI.sh failed sh -n. Aborting."; rm -f "$STATE/.rh.patched"; exit 1
fi
cnt=$(grep -c "mst2-carplay-vc.jar" "$STATE/.rh.patched" 2>/dev/null)
if [ "$cnt" != 1 ]; then
    echo "FAIL: expected 1 bridge line, got $cnt. Aborting."; rm -f "$STATE/.rh.patched"; exit 1
fi
if [ "$USE_COVER" = yes ]; then
    cbcnt=$(grep -c "cpvc-cover-bridge" "$STATE/.rh.patched" 2>/dev/null)
    if [ "$cbcnt" != 1 ]; then
        echo "FAIL: expected 1 cover-bridge keepalive line, got $cbcnt. Aborting."; rm -f "$STATE/.rh.patched"; exit 1
    fi
fi
mv "$STATE/.rh.patched" "$RUN_HMI" || { echo "FAIL: could not install patched runHMI.sh. Aborting."; exit 1; }
...
# 7) debug logging: --debug -> on, else off
if [ "$DEBUG" = yes ]; then
