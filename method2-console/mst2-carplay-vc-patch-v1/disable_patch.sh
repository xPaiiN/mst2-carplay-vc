#!/bin/sh
# disable_patch.sh -- MANUAL rollback: restore runHMI, remove the jar + cover-bridge, restore
# iap2.cfg, wipe runtime state. Safe to run for any variant (lite never deployed a cover-bridge,
# so those steps are simply no-ops). Whitelist-safe self-dir resolution.
case "$0" in
    */*) CPVC_SELFDIR="${0%/*}" ;;
    *)   CPVC_SELFDIR="." ;;
esac
