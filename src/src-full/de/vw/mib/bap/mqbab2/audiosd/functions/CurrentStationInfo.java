...
ExboxServiceListener {
    ...
    // Static INSTANCE so the Bridge can force-call process(-1) via reflection after a
    // PPS-cache update. process() carries its own 200 ms throttle internally.
    public static volatile CurrentStationInfo INSTANCE = null;
    ...
    private static final int TIMER_UPDATE_TIME = 200;
    private Timer timer = null;
    private static final int TIMER_ACTION_NOTHING = 0;
    private static final int TIMER_ACTION_UPDATE_DATA = 1;
    private static final int TIMER_ACTION_STOP_IGNORE_ENTERTAINMENT_SUPRESSION = 2;
    private static final int TIMER_UPDATE_SUPPRESSION_TIME = 2000;
    private Timer _suppressionTimer;
    private static final String MIRROR_LINK_NAME = "MirrorLink™";
    private static final String GOOGLE_GAL_NAME = "Android Auto";
    private static final String BAIDU_CARLIFE_NAME = "Baidu CarLife";
    private static final String TV_TUNER_SOURCE_AV_PI = "AV";
    private static final String I18N_FILTERCRITERIA_UNKNOWN_ARTIST_STRING = "filterCriteria.unknownArtist";
    private static final String I18N_FILTERCRITERIA_UNKNOWN_ALBUM_STRING = "filterCriteria.unknownAlbum";
    private static final String I18N_FILTERCRITERIA_NOW_PLAYING_FOLDER_STRING = "filterCriteria.nowPlaying";
    protected static final int[] RADIO_LISTENER_IDS = new int[]{1410, 1339, 1324, 1320, 1321, 1371, 1372, 3404, 3403, 1443, 1436, 2323, 1409, 1423, 1356, 1357, 1352, 1354, 2410015, 2410014, 1445, 1446, 1366, 1426, 1454, 1456, 1455, 1308, 3962};
    protected static final int[] MEDIA_LISTENER_IDS = new int[]{8, 3520, 124, 125, 138, 2849, 58};
    protected static final int[] TV_TUNER_LISTENER_IDS = new int[]{2314};
    protected static final int[] SMARTPHONE_INTEGRATION_LISTENER_IDS = new int[]{2910018};
    protected static final int[] NAVIGATION_LISTENER_IDS = new int[]{2389, 2388};
    protected static final int[] SOUND_LISTENER_IDS = new int[]{1470};
    protected static final int[] EXBOX_LISTENER_IDS = new int[]{-2147478647};
    static Class class$de$vw$mib$bap$mqbab2$generated$audiosd$serializer$CurrentStationInfo_Status;


    public BAPEntity init(BAPStageInitializer bAPStageInitializer) {
        // init() fires earlier and once per BAP-stage lifecycle, before initialize().
        INSTANCE = this;
    ...
    private void setArtistAndAlbum(CurrentStationInfo_Status currentStationInfo_Status) {
        ...
        if (string != null && string.compareTo(I18N_FILTERCRITERIA_UNKNOWN_ARTIST_STRING) == 0) {
        ...
        if (string2 != null && string2.compareTo(I18N_FILTERCRITERIA_UNKNOWN_ALBUM_STRING) == 0) {
    ...
    private String getActivePlayingFolderName() {
        ...
        return string.compareTo(I18N_FILTERCRITERIA_NOW_PLAYING_FOLDER_STRING) == 0 ? "" : string;
    }
    ...
    private void setStationInfoForMirrorLink(CurrentStationInfo_Status currentStationInfo_Status) {
        int connectionType = this.getSmartphoneIntegrationService().getSmarphoneIntegrationActiveConnectionType();
        // For CarPlay (case 2): read the Bridge PPS-cache (filled by the 2 Hz PPS-poll thread)
        // and push title/artist/album into all four BAP slots. Stock MediaService acts as
        // fallback if the Bridge cache is still empty (e.g. immediately after boot).
        if (connectionType == 2) {
            // Text persists during calls (same as cover); only skip/source-change updates it.
            String title = "", artist = "", album = "";
            try {
                title = com.mst2.carplay.bridge.Mst2NowPlayingBridge.getCachedTitle();
                artist = com.mst2.carplay.bridge.Mst2NowPlayingBridge.getCachedArtist();
                album = com.mst2.carplay.bridge.Mst2NowPlayingBridge.getCachedAlbum();
            } catch (Throwable t) { /* swallow */ }
            // Fallback to Stock MediaService when the Bridge cache has not been populated yet.
            if (title == null || title.length() == 0) {
                try {
                    de.vw.mib.bap.mqbab2.common.api.media.datatypes.MediaCurrentTrackInfo info =
                        this.getMediaService().getCurrentTrackInfo();
                    if (info != null) {
                        String t2 = info.getCurrentTrackTitle();
                        if (t2 != null && t2.length() > 0 && !isLdeJunk(t2)) title = t2;
                        String a2 = info.getCurrentTrackArtist();
                        if (a2 != null && a2.length() > 0 && !isLdeJunk(a2)) artist = a2;
                        String al2 = info.getCurrentTrackAlbum();
                        if (al2 != null && al2.length() > 0 && !isLdeJunk(al2)) album = al2;
                    }
                } catch (Throwable t) { /* swallow */ }
            }
            // Lde-junk pre-filter: never push a CFR-Decompiler-Artefakt into the VC.
            if (isLdeJunk(title)) title = "";
            if (isLdeJunk(artist)) artist = "";
            if (isLdeJunk(album)) album = "";
            // Timestamp on the 4th VC line (h3): pi_Type 72->70 + qi=time. Cover-preserving.
            String mst2Ts = "";
            try { mst2Ts = com.mst2.carplay.bridge.Mst2NowPlayingBridge.getCurrentTimestamp(); }
            catch (Throwable mst2T) { mst2Ts = ""; }
            if (mst2Ts == null) mst2Ts = "";
            // pi (title): pi_Type 70 (radio class) when the time line is shown, otherwise 72 (media).
            currentStationInfo_Status.primaryInformation.setContent(title != null ? title : "");
            int mst2PiType = (mst2Ts.length() > 0) ? 70 : 72;
            currentStationInfo_Status.pi_Type = (title != null && title.length() > 0) ? mst2PiType : 0;
            currentStationInfo_Status.pi_Id = 0;
            // si (artist): unchanged.
            currentStationInfo_Status.secondaryInformation.setContent(artist != null ? artist : "");
            currentStationInfo_Status.si_Type = (artist != null && artist.length() > 0) ? 73 : 0;
            // ti (album): unchanged.
            currentStationInfo_Status.tertiaryInformation.setContent(album != null ? album : "");
            currentStationInfo_Status.ti_Type = (album != null && album.length() > 0) ? 74 : 0;
            // qi (4th slot): the time (qi_Type=1 = media folder) when present, otherwise empty.
            if (mst2Ts.length() > 0) {
                currentStationInfo_Status.quaternaryInformation.setContent(mst2Ts);
                currentStationInfo_Status.qi_Type = 1;
            } else {
                currentStationInfo_Status.quaternaryInformation.setContent("");
                currentStationInfo_Status.qi_Type = 0;
            }
        ...
        switch (connectionType) {
            case 1: {
                string = MIRROR_LINK_NAME;
                n = 72;
                break;
            }
            case 3: {
                string = GOOGLE_GAL_NAME;
                n = 72;
                break;
            }
            case 4: {
                string = BAIDU_CARLIFE_NAME;
                n = 72;
                break;
            }
            default: {
    ...
    private void setCurrentStationInfoForTvTuner(CurrentStationInfo_Status currentStationInfo_Status) {
        ...
        switch (tvTunerService.getCurrentTvTunerSource()) {
            case 1: {
                currentStationInfo_Status.primaryInformation.setContent(TV_TUNER_SOURCE_AV_PI);
                break;
            }
            default: {
    ...
    public void initialize(boolean bl) {
        ...
        // Re-expose INSTANCE on each reattach.
        INSTANCE = this;
    }
    ...
    protected void setAutostoreRunning(Boolean bl) {
        boolean bl2 = bl.booleanValue();
        if (this.autostoreRunning != bl2) {
            this.autostoreRunning = bl2;
            this.process(-1);
        }
    }
    ...
    static /* synthetic */ Class class$(String string) {
        try {
            ...
            // Java 1.4: NoClassDefFoundError.initCause returns Throwable, not Error
            NoClassDefFoundError err = new NoClassDefFoundError(classNotFoundException.getMessage());
            throw err;
        }
    }

    // Pre-filter strings that look like a CFR-Decompiler-Artefakt leaked through a Stock-Bytecode
    // round-trip. Observed live: "([Lde/vw/mib/cio/CioExecutor;)V". "CioExecutor" is an internal
    // MIB class name and cannot appear in a real Apple Music title/artist/album. New junk
    // variants should be added explicitly rather than via a broader regex.
    private static boolean isLdeJunk(String s) {
        if (s == null) return false;
        return s.indexOf("CioExecutor") >= 0;
    }
}
