...
TvTunerServiceListener {
    ...
    private static final int CURRENT_STATION_IN_RECEPTIONLIST_BAP_POS_ID = 1;
    private static final int OFFSET_FOR_REF_HANDLE = 1;
    private static final int NO_HANDLE_REF_EXISTS = 0;
    private static final int INVALID_HANDLE_REF = -1;
    private static final long INVALID_DAB_ID = -1L;
    private static final int INVALID_INDEX = -1;
    private static final int DAB_NO_ABS_POS_ID_EXITS = -1;
    private static final int INVALID_BAP_POS_ID = -1;
    private static final int INDEX_ID = 0;
    private static final int INDEX_ID_ABS_POSITION = 1;
    private static final int INDEX_ENSEMBLE_ID = 2;
    private static final int INDEX_PARENT_ABS_POSITION = 2;
    private static final int INDEX_ENSEMBLE_ID_ABS_POS = 3;
    protected static final int[] MEDIA_LISTENER_IDS = new int[]{810013, 810015};
    protected static final int[] RADIO_LISTENER_IDS = new int[]{1324, 2486, 1412};
    protected static final int[] TV_TUNER_LISTENER_IDS = new int[]{2662};
    protected static final int[] SOUND_LISTENER_IDS = new int[]{1470};
    static Class class$de$vw$mib$bap$mqbab2$generated$audiosd$serializer$CurrentStation_Handle_Status;

    // synth handle MUST match the CoverArt cover-push handle (1).
    private static final int CARPLAY_SYNTH_FSGHANDLE = 1;
    ...
    /**
     * Synthetic CarPlay handle. Avoids the stock CoverArt "currentStationHandle != -1" block.
     * Handle=1 must match the CoverArt cover-push handle so the cluster sees a
     * consistent session ID.
     */
    private void setCurrentStationHandleForCarPlaySynth(CurrentStation_Handle_Status currentStation_Handle_Status) {
        currentStation_Handle_Status.fsgHandle = CARPLAY_SYNTH_FSGHANDLE;
    ...
    private void setCurrentStationHandleData(CurrentStation_Handle_Status currentStation_Handle_Status) {
        switch (this.getSoundService().getCurrentAudioComponent()) {
            ...
            // Stock lacks case 6 (CarPlay) -> default branch -> fsgHandle=-1 -> stock CoverArt block.
            // We set a synth handle (1, or the media handle if available) so the cover-push
            // fires handle-aligned. Applies only to CarPlay (component 6); otherwise byte-stock.
            case 6: {
                if (this.getMediaService().isBapMediaBrowserAvailable()) {
                    this.setCurrentStationHandleForMedia(currentStation_Handle_Status);
                    if (currentStation_Handle_Status.fsgHandle == -1) {
                        this.setCurrentStationHandleForCarPlaySynth(currentStation_Handle_Status);
                    }
                } else {
                    this.setCurrentStationHandleForCarPlaySynth(currentStation_Handle_Status);
                }
