...
public final class Mst2NowPlayingBridge {
    ...
    private static final long REFRESH_TICK_POLL_MS     = 250L;    // poll the displayed timestamp 4x/s

    // Timestamp: hour-overflow safe-cap (max "99:59:59") + PPS-stale threshold.
    private static final long HH_OVERFLOW_CAP = 99L;
    private static final long PPS_TICK_STALE_MS = 5000L;
    // Resync-jitter tolerance. A raw PPS elapsed sample carries up to one source-update-interval of
    // discovery-latency noise (the CarPlay source pushes elapsed every ~460 ms, uncorrelated with the
    // 500 ms poll phase). Snapping cachedElapsedMs to every raw sample would inject that jitter into the
    // extrapolation -> irregular ~0.8-1.2 s display cadence. A sample within this tolerance of our own
    // running prediction is routine jitter (kept smooth); a larger deviation (seek/skip/track-change)
    // resyncs the anchor. 700 ms sits above the ~460 ms routine max and below any real seek.
    private static final long ELAPSED_RESYNC_TOLERANCE_MS = 700L;
    // Timestamp lag compensation. The VC lags ~1 s behind the MU; +500 ms moves the display closer to
    // real time while still staying safely behind (never ahead). Added only in the Playing branch.
    private static final long ELAPSED_PREDICT_OFFSET_MS = 500L;
    // Timestamp format: true = "[h:]mm:ss / -[h:]mm:ss" (elapsed / -remaining, countdown); false = elapsed/total.
    // Hours appear per value only when that value >= 1 h (e.g. "2:59:35 / -21:45"); < 1 h stays "m:ss".
    // static final -> javac keeps only one format branch in the bytecode.
    private static final boolean TS_REMAINING = true;
    ...
    // Timestamp state (PPS-fed + locally interpolated between polls).
    private static volatile long cachedDurationMs = 0L;
    private static volatile long cachedElapsedMs = 0L;
    private static volatile long lastPpsElapsedTickMs = 0L;
    ...
    // Timestamp getter for the Shadow CurrentStationInfo CarPlay path (quaternaryInformation).
    // Returns "[h:]mm:ss / -[h:]mm:ss" (elapsed / -remaining) or elapsed/total, or "" if there
    // is no duration. While Playing, the wall-clock delta since the last PPS tick is added to elapsed
    // (local 1 Hz interpolation) + ELAPSED_PREDICT_OFFSET_MS; while Pause or PPS-stale it is frozen.
    public static String getCurrentTimestamp() {
        long dur = cachedDurationMs;
        if (dur <= 0L) return "";
        long elapsedNow;
        long now = System.currentTimeMillis();
        long tick = lastPpsElapsedTickMs;
        boolean ppsStale = (tick <= 0L) || ((now - tick) > PPS_TICK_STALE_MS);
        if (cachedPlaying && !ppsStale) {
            long deltaSinceTick = now - tick;
            if (deltaSinceTick < 0L) deltaSinceTick = 0L; // NTP-clock-skew defense
            elapsedNow = cachedElapsedMs + deltaSinceTick + ELAPSED_PREDICT_OFFSET_MS;
            if (elapsedNow > dur) elapsedNow = dur;
        } else {
            elapsedNow = cachedElapsedMs; // Pause or PPS-stale: freeze
        }
        if (elapsedNow < 0L) elapsedNow = 0L;
        if (TS_REMAINING) {                       // elapsed / -remaining (countdown, Apple style)
            // Derive remaining from elapsed's WHOLE second so both halves flip together, exactly once
            // per whole elapsed-second (avoids the ~0.5 s left/right desync on non-round durations).
            long remainingSec = (dur / 1000L) - (elapsedNow / 1000L);
            if (remainingSec < 0L) remainingSec = 0L;   // defensive (elapsedNow is already clamped to dur)
            return formatTime(elapsedNow) + " / -" + formatTime(remainingSec * 1000L);
        }
        return formatTime(elapsedNow) + " / " + formatTime(dur);   // elapsed / total duration (static)
    }

    // Adaptive: "h:mm:ss" when the value is >= 1 h, else "m:ss" (unchanged for < 1 h content).
    // Hours are not zero-padded; minutes are 2-digit only when hours are present; seconds always 2-digit.
    private static String formatTime(long ms) {
        long totalSec = ms / 1000L;
        long h = totalSec / 3600L;
        long m = (totalSec % 3600L) / 60L;
        long s = totalSec % 60L;
        if (h > HH_OVERFLOW_CAP) { h = HH_OVERFLOW_CAP; m = 59L; s = 59L; } // safe-cap "99:59:59"
        StringBuffer sb = new StringBuffer(9);
        if (h > 0L) {
            sb.append(h);
            sb.append(':');
            if (m < 10L) sb.append('0');   // mm 2-digit only when hours present
            sb.append(m);
        } else {
            sb.append(m);                   // m unpadded without hours (= old m:ss)
        }
        sb.append(':');
        if (s < 10L) sb.append('0');        // ss always 2-digit
        sb.append(s);
        return sb.toString();
    }
    ...
    public static void resetAttach() {
        ...
        // clear timestamp state (ghost-time mitigation)
        cachedDurationMs = 0L;
        cachedElapsedMs = 0L;
        lastPpsElapsedTickMs = 0L;
        cachedPlaying = false;
                    ...
                    try {
                        ...
                        if (!content.equals(lastContent)) {
                            ...
                            if (content.length() > 0) {
                                ...
                                // Timestamp: parse position/duration/status + note the tick.
                                long newDurationMs = parseLongOrZero(extract(content, "MediaItemPlaybackDurationInMilliseconds::"));
                                long newElapsedMs  = parseLongOrZero(extract(content, "PlaybackElapsedTimeInMilliseconds::"));
                                boolean newPlaying = "Playing".equalsIgnoreCase(extract(content, "PlaybackStatus::"));
                                long nowTickMs = System.currentTimeMillis();
                                // Resync-jitter fix: accept the raw sample only if it deviates from our own
                                // running prediction by more than the tolerance (a real discontinuity);
                                // otherwise keep the smooth extrapolated value. lastPpsElapsedTickMs always
                                // advances, so staleness detection is unaffected.
                                if (cachedPlaying && newPlaying && lastPpsElapsedTickMs > 0L) {
                                    long predictedElapsedMs = cachedElapsedMs + (nowTickMs - lastPpsElapsedTickMs);
                                    long signedDiffMs = newElapsedMs - predictedElapsedMs;
                                    long elapsedDiffMs = signedDiffMs < 0L ? -signedDiffMs : signedDiffMs;
                                    boolean smoothed = elapsedDiffMs <= ELAPSED_RESYNC_TOLERANCE_MS;
                                    if (DEBUG) dlog("ts-resync: raw=" + newElapsedMs + " pred=" + predictedElapsedMs
                                                    + " diff=" + signedDiffMs + (smoothed ? " SMOOTH" : " HARD"));
                                    if (smoothed) newElapsedMs = predictedElapsedMs;
                                }
                                cachedDurationMs = newDurationMs;
                                cachedElapsedMs = newElapsedMs;
                                cachedPlaying = newPlaying;
                                lastPpsElapsedTickMs = nowTickMs;
    ...
    private static void startBapRefreshTicker() {
        ...
        Thread t = new Thread(new Runnable() {
            public void run() {
                // Timestamp sync: drive the VC re-render value-driven. Poll the displayed timestamp 4x/s
                // and refresh the instant the m:ss string changes, so elapsed+remaining advance smoothly at
                // a clean 1 Hz aligned to the real second boundary. A periodic safety refresh preserves the
                // title push net when the timestamp is static (paused, PPS-stale, or no track).
                String lastTs = "";
                long lastForcedMs = 0L;
                long lastTsChangeMs = 0L;   // diag: wall-clock of the last displayed-value change (cadence probe)
                while (true) {
                    try {
                        if (attached && !isCallActive()) {
                            long now = System.currentTimeMillis();
                            String ts = "";
                            try { ts = getCurrentTimestamp(); } catch (Throwable t2) { ts = ""; }
                            if (ts == null) ts = "";
                            boolean tsChanged = !ts.equals(lastTs);
                            if (tsChanged || (now - lastForcedMs) >= REFRESH_TICK_INTERVAL_MS) {
                                if (DEBUG && tsChanged && ts.length() > 0 && lastTsChangeMs > 0L)
                                    dlog("ts-cadence: dt=" + (now - lastTsChangeMs) + " disp=" + ts);
                                if (tsChanged) lastTsChangeMs = now;
                                lastTs = ts;
                                lastForcedMs = now;
                                triggerBapRefresh();
                            }
                        }
                        Thread.sleep(REFRESH_TICK_POLL_MS);
                    } catch (InterruptedException ie) { return; }
                    catch (Throwable th) {
            ...
            cachedStateListener = new DSIServiceStateListener() {
                ...
                public void unregistered(String svcName, int channel) {
                    if (svcName != null && svcName.equals(dsiCarplayClass.getName())) {
                        ...
                        lastValidUpdateMs = 0L;
                        // clear timestamp state on disconnect
                        cachedDurationMs = 0L;
                        cachedElapsedMs = 0L;
                        lastPpsElapsedTickMs = 0L;
                        cachedPlaying = false;
