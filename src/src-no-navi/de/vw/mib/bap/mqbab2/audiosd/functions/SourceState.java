...
RadioServiceListener {
    ...
    private static final int MANUAL_TUNE_STATE_IDLE = 0;
    private static final int MANUAL_TUNE_STATE_ENTER = 1;
    private static final int MANUAL_TUNE_STATE_ACTIVE = 2;
    private static final int MANUAL_TUNE_STATE_LEAVE_PENDING = 3;
    private static final int MANUAL_TUNE_STATE_LEAVE = 4;
    ...
    protected static final int[] MEDIA_LISTENER_IDS = new int[]{8, 3520, 44, 85, 85, 41, 46, 29};
    protected static final int[] RADIO_LISTENER_IDS = new int[]{1438, 1446, 1445};
    static Class class$de$vw$mib$bap$mqbab2$generated$audiosd$serializer$SourceState_Status;

    // Prio 13: Static-Handle, damit die Bridge nach einem Shuffle-Wechsel process(-1) reflektiv
    // aufrufen kann (analog CurrentStationInfo.INSTANCE).
    public static volatile SourceState INSTANCE = null;
    ...
    private SourceState_Status computeSourceStateStatus() {
        ...
        // Prio 13: CarPlay (audioComponent==6) Shuffle-Injektion. Stock zwingt stateInfo fuer
        // Component 6 hart auf 0 (CarPlay erreicht den Media-Pfad nie), darum erscheint das Cluster-
        // Mix/Shuffle-Icon fuer CarPlay nie. Meldet das iPhone Shuffle (von der Bridge aus dem
        // PPS-Feld PlaybackShuffleMode:: erfasst), setzen wir stateInfo=2 (Mix/Shuffle only). USB/SD
        // (Component 2) und Radio (Component 1) bleiben unberuehrt -> Stock-Shuffle/Repeat 100 %.
        if (this.getSoundService().getCurrentAudioComponent() == 6
            && com.mst2.carplay.bridge.Mst2NowPlayingBridge.isCarplayShuffleOn()) {
            sourceState_Status.stateInfo = 2;
        }
